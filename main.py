...
def main():
    ...
    bullet_big_supply = supply.Bullet_big_Supply(bg_size)
    ...
    THREE_BULLET_TIME = pygame.USEREVENT + 2
    # 标志是否使用三重子弹
    ...
    # 解除我方无敌状态定时器
    INVINCIBLE_TIME = pygame.USEREVENT + 3
    ...
    while running:
        ...
        if life_num and not paused:
            ...
            # 绘制三重子弹补给并检测是否获得
            if bullet_big_supply.active:
                bullet_big_supply.move()
                screen.blit(bullet_big_supply.image, bullet_big_supply.rect)
                if pygame.sprite.collide_mask(bullet_big_supply, me):
                    get_bullet_sound.play()
                    is_three_bullet = True
                    pygame.time.set_timer(THREE_BULLET_TIME, 18 * 1000)
                    bullet_big_supply.active = False
            ...
            for each in big_enemies:
                if each.active:
                    ...
                    if energy_remain > 0.2:
                        ...
                        enery_color = RED
                    pygame.draw.line(screen, enery_color, \
                                     (each.rect.left, each.rect.top - 5), \
